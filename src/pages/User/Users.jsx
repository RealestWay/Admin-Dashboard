...
const Users = () => {
  ...
  const handleDelete = async () => {
    const url = `https://realestway-backend.up.railway.app/users/${userId}`;
    const headers = {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatus(
          `Failed to delete user ${userId}. Status: ${response.status}`
        );
      }
    } catch (error) {
  ...
  const rowsPerPage = 5;
  const users = allUsers.slice(page * rowsPerPage, (page + 1) * rowsPerPage);
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 1, borderRadius: "50px" }}>
          ...
          <button
            onClick={() => setPage(page + 1)}
            disabled={(page + 1) * rowsPerPage >= allUsers.length}
            style={{
              background: "#EAEEF4",
              border: "none",
              cursor: "pointer",
...
const Body = ({ id, phone, email, name, rented, index }) => {
      ...
      <td style={{ padding: "8px", position: "relative" }}>
        ...
        {open ? (
          ...
          >
            ...
            <p style={{ borderBottom: "1px solid gray", padding: "6px 18px" }}>
              Suspend
            </p>
            <p style={{ borderBottom: "1px solid gray", padding: "6px 18px" }}>
              Delete
